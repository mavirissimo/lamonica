...
if ( ! class_exists( 'Easyjet_Playlist_Slider_Widget' ) ) {

	/**
	 * Class Easyjet_Playlist_Slider_Widget.
	 */
	class Easyjet_Playlist_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Playlist Slider', 'easyjet' );
			$this->widget_description = esc_html__( 'Display playlist slider on your site.', 'easyjet' );
			$this->widget_id          = apply_filters( 'easyjet_playlist_slider_widget_ID', 'easyjet_widget_playlist_slider' );
			$this->widget_cssclass    = apply_filters( 'easyjet_playlist_slider_widget_cssclass', 'widget-playlist-slider' );
			$this->utility            = easyjet_utility()->utility;
			$this->settings = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Playlist Slider', 'easyjet' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', 'easyjet' ),
				),
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width ( px, %, rem )', 'easyjet' ),
				),
				'height' => array(
					'type' => 'text',
					'value' => '525',
					'label' => esc_html__( 'Slider height ( px, rem )', 'easyjet' ),
				),
				'thumbnail_controls' => array(
					...
					'value'   => true,
					'options' => array(
						false => esc_html__( 'Hide', 'easyjet' ),
						true  => esc_html__( 'Buttons', 'easyjet' ),
					),
					'label'       => esc_html__( 'Select Thumbnail Controls', 'easyjet' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'easyjet' ),
				),
				'slider_controls' => array(
					...
					'label'       => esc_html__( 'Select Slider Controls', 'easyjet' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'easyjet' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Title words length ( Set 0 to hide title. )', 'easyjet' ),
				),
				'thumb_title_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Thumbnail title words length ( Set 0 to hide title. )', 'easyjet' ),
				),
				'mate_data' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'date'     => esc_html__( 'Date', 'easyjet' ),
						'author'   => esc_html__( 'Author', 'easyjet' ),
						'comments' => esc_html__( 'Comment count', 'easyjet' ),
						'category' => esc_html__( 'Category', 'easyjet' ),
						'post_tag' => esc_html__( 'Tag', 'easyjet' ),
					),
					'label' => esc_html__( 'Display post meta data', 'easyjet' ),
				),
				'thumb_mate_data' => array(
					...
					'label' => esc_html__( 'Display thumbnail post meta data', 'easyjet' ),
				),
			);
			...
			$this->layout_settings = apply_filters( 'easyjet_playlist_slider_size', array(
		...
		public function the_thumbnail_view() {
			$thumbnails_view_dir = easyjet_get_locate_template( 'inc/widgets/playlist-slider/views/playlist-thumbnails-view.php' );
		...
		public function the_slides_view() {
			$slides_view_dir = easyjet_get_locate_template( 'inc/widgets/playlist-slider/views/playlist-slides-view.php' );
		...
		public function widget( $args, $instance ) {
			...
			if ( $this->posts ) {
				$slider_settings = array(
					...
					'buttons'            => apply_filters( 'easyjet_playlist_buttons', false ),
				...
				$holder_view_dir = easyjet_get_locate_template( 'inc/widgets/playlist-slider/views/playlist-holder-view.php' );
...
add_action( 'widgets_init', 'easyjet_register_playlist_slider_widget' );
/**
 * Register playlist-slider widget.
 */
function easyjet_register_playlist_slider_widget() {
	register_widget( 'Easyjet_Playlist_Slider_Widget' );
}
