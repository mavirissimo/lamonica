...
if ( ! class_exists( 'Easyjet_Custom_Posts_Widget' ) ) {

	/**
	 * Class Easyjet_Custom_Posts_Widget.
	 */
	class Easyjet_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', 'easyjet' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', 'easyjet' );
			$this->widget_id          = apply_filters( 'easyjet_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( 'easyjet_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = easyjet_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', 'easyjet' ),
				...
				'terms_type' => array(
					...
					'options' => array(
						...
						'post_format'   => array(
							'label' => esc_html__( 'Post Format', 'easyjet' ),
				...
				'post_format' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select post format', 'easyjet' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', 'easyjet' ),
				...
				'post_offset' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Offset post', 'easyjet' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					'value'      => '10',
				...
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'easyjet' ),
				),
				'meta_data' => array(
					'type'    => 'checkbox',
					'value'   => array(
					...
					'options' => array(
						...
						'comment_count' => esc_html__( 'Comment count', 'easyjet' ),
						'category'      => esc_html__( 'Category', 'easyjet' ),
						'post_tag'      => esc_html__( 'Tag', 'easyjet' ),
						'more_button'   => esc_html__( 'More Button', 'easyjet' ),
					),
				...
				'button_text' => array(
					...
					'label' => esc_html__( 'Post read more button label', 'easyjet' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$footer_columns = get_theme_mod( 'footer_widget_columns', easyjet_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			if ( $posts ) {
				...
				foreach ( $posts as $post ) {
					...
					if (
						...
						$image_size = 'easyjet-thumb-480-271';
					}
	...
	add_action( 'widgets_init', 'easyjet_register_custom_posts_widget' );
	/**
	 * Register custom-posts widget.
	 */
	function easyjet_register_custom_posts_widget() {
		register_widget( 'Easyjet_Custom_Posts_Widget' );
	}
}
