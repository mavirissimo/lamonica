...
if ( ! class_exists( 'Easyjet_Featured_Posts_Block_Widget' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class Easyjet_Featured_Posts_Block_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Featured Posts Block', 'easyjet' );
			$this->widget_description = esc_html__( 'This widget displays latest posts', 'easyjet' );
			$this->widget_id          = 'easyjet_widget_featured_posts_block';
			...
			foreach( $layouts as $id => $layout ) {
				$layout_options[ $id ] = array(
					'label'   => $layout['name'],
					'img_src' => EASYJET_THEME_URI . '/assets/images/admin/widgets/featured-posts-block/' . $id . '.svg',
				);
			}
			...
			$this->settings = array(
				...
				'layout' => array(
					...
					'label'   => esc_html__( 'Layout', 'easyjet' ),
					'options' => $layout_options,
				),
				'posts_ids' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Posts IDs (Optional)', 'easyjet' ),
				),
				'checkboxes'     => array(
					'type'  => 'checkbox',
					'label' => esc_html__( 'Post Meta', 'easyjet' ),
					'value' => array(
						'title'      => 'true',
						'excerpt'    => 'true',
						'categories' => 'true',
					...
					'options' => array(
						'title'      => esc_html__( 'Show title', 'easyjet' ),
						'excerpt'    => esc_html__( 'Show excerpt', 'easyjet' ),
						'categories' => esc_html__( 'Show categories', 'easyjet' ),
						'tags'       => esc_html__( 'Show tags', 'easyjet' ),
						'author'     => esc_html__( 'Show author', 'easyjet' ),
						'date'       => esc_html__( 'Show date', 'easyjet' ),
					),
				),
				'title_length' => array(
					'type'      => 'stepper',
					'value'     => 12,
					'min_value' => 1,
					'label'     => esc_html__( 'Title length (chars)', 'easyjet' ),
				),
				'excerpt_length' => array(
					'type'      => 'stepper',
					'value'     => 15,
					'min_value' => 1,
					'label'     => esc_html__( 'Excerpt length (words)', 'easyjet' ),
				),
			);
		...
		public function render_layout( $options = array() ) {
			...
			$item_template = easyjet_get_locate_template( 'inc/widgets/featured-posts-block/views/item.php' );
			 ...
			 */
			$query = apply_filters( 'easyjet_featured_posts_block_query', $query, $this->instance );
		...
		public function prepare_data( $data, $layout ) {
			...
			$container_template = easyjet_get_locate_template( 'inc/widgets/featured-posts-block/views/container.php' );
		...
		public function get_layouts() {
			return array(
				'layout-1' => array(
					'name'   => esc_html__( 'Type #1', 'easyjet' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-2' => array(
					'name'   => esc_html__( 'Type #2', 'easyjet' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-3' => array(
					'name'   => esc_html__( 'Type #3', 'easyjet' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-4' => array(
					'name'   => esc_html__( 'Type #4', 'easyjet' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-5' => array(
					'name'   => esc_html__( 'Type #5', 'easyjet' ),
					'amount' => 3,
					'markup' => array(
						array(
							'type'  => 'item',
	...
	add_action( 'widgets_init', 'easyjet_register_featured_posts_block_widget' );

	if ( false === function_exists( 'easyjet_register_featured_posts_block_widget' ) ) {
		/**
		 * Register featured posts block widget.
		 */
		function easyjet_register_featured_posts_block_widget() {
			register_widget( 'Easyjet_Featured_Posts_Block_Widget' );
		}
	}
}
