...
class Easyjet_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'easyjet' );
		$this->widget_id          = 'easyjet_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'easyjet' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'easyjet' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Newsletter signup', 'easyjet' ),
				'label' => esc_html__( 'Subscribe Title', 'easyjet' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'easyjet' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your e-mail', 'easyjet' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'easyjet' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'easyjet' ),
				'label' => esc_html__( 'Subscribe submit label', 'easyjet' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'easyjet' ),
				'label' => esc_html__( 'Subscribe success', 'easyjet' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'easyjet' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow us', 'easyjet' ),
				'label' => esc_html__( 'Follow Title', 'easyjet' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'easyjet' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'easyjet' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'easyjet' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'easyjet' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'easyjet' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'easyjet' ),
				'value'   => 'center',
				'options' => array(
			...
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'easyjet' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'easyjet' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'easyjet' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'easyjet' ),
				'value'   => 'inherit',
				'options' => array(
			...
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'easyjet' ),
				'value'  => '#000000',
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_easyjet_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_easyjet_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'easyjet' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'easyjet' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'easyjet' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'easyjet' ),
		);

		$this->nonce = wp_create_nonce( 'easyjet-subscribe-nonce' );
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'easyjet' );
		}
	...
	public function get_social_nav() {
		return easyjet_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = easyjet_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'easyjet-subscribe-nonce' ) ) {
...
add_action( 'widgets_init', 'easyjet_register_subscribe_follow_widgets' );
/**
 * Register subscribe-follow widget.
 */
function easyjet_register_subscribe_follow_widgets() {
	register_widget( 'Easyjet_Subscribe_Follow_Widget' );
}
