 ...
 * @package Easyjet
 */
if ( ! class_exists( 'Easyjet_About_Author_Widget' ) ) {

	/**
	 * Class Easyjet_About_Author_Widget.
	 */
	class Easyjet_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'easyjet widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'easyjet' );
			$this->widget_id          = 'easyjet_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'easyjet' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'easyjet' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'easyjet' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'easyjet' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'easyjet' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'easyjet' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'easyjet' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'easyjet' ),
					'label' => esc_html__( 'Link label', 'easyjet' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'easyjet' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'easyjet-thumb-m' );
...
add_action( 'widgets_init', 'easyjet_register_about_author_widgets' );
/**
 * Register about-author widget.
 */
function easyjet_register_about_author_widgets() {
	register_widget( 'Easyjet_About_Author_Widget' );
}
