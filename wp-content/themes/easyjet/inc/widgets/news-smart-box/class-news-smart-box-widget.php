...
/**
 * Class Easyjet_News_Smart_Box_Widget
 */
class Easyjet_News_Smart_Box_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_id          = 'easyjet_widget_news_smart_box';
		$this->widget_name        = esc_html__( 'News Smart Box', 'easyjet' );
		$this->widget_description = esc_html__( 'News Smart Box', 'easyjet' );
		$this->utility            = easyjet_utility()->utility;
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'News Smart Box', 'easyjet' ),
			...
			'layout_type' => array(
				...
				'options' => array(
					'layout_type_1' => array(
						'label'   => esc_html__( 'Layout type 1', 'easyjet' ),
						'img_src' => EASYJET_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-1.svg',
					),
					'layout_type_2' => array(
						'label'   => esc_html__( 'Layout type 2', 'easyjet' ),
						'img_src' => EASYJET_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-2.svg',
					),
					'layout_type_3' => array(
						'label'   => esc_html__( 'Layout type 3', 'easyjet' ),
						'img_src' => EASYJET_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-3.svg',
					),
				),
			...
			'categories' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'category', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary categories', 'easyjet' ),
				'multiple'         => true,
				'placeholder'      => esc_html__( 'Select categories', 'easyjet' ),
				'master'           => 'terms_type_category',
			),
			'current_tag' => array(
				'type'             => 'select',
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select tag', 'easyjet' ),
				'placeholder'      => esc_html__( 'Select tag', 'easyjet' ),
			...
			'tags' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary tags', 'easyjet' ),
				'multiple'         => true,
			...
			'trim_title_chars' => array(
				...
				'min_value'  => 0,
				'step_value' => 1,
				'label'      => esc_html__( 'Title chars trimmed count', 'easyjet' ),
			),
			...
			'current_title_visibility' => array(
				...
				'label' => esc_html__( 'Display current navigation title', 'easyjet' ),
			),
			'date_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display date', 'easyjet' ),
			),
			'author_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display author', 'easyjet' ),
			),
			'comment_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display comments', 'easyjet' ),
			),
			'category_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display categories', 'easyjet' ),
			),
			'tags_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display tags', 'easyjet' ),
			),
		...
		add_filter( 'easyjet_theme_script_variables', array( $this, 'add_new_smart_box_nonce_var' ) );
	...
	public function add_new_smart_box_nonce_var( $vars = array() ) {
		$vars['new_smart_box_nonce'] = wp_create_nonce( 'easyjet-new-smart-box-nonce' );
	...
	public function get_query_items( $query_args = array() ) {
		...
		$defaults_query_args = apply_filters( 'easyjet_news_smart_box_default_query_args', array(
			'post_type'      => 'post',
			'orderby'        => 'date',
			'order'          => 'DESC',
			'posts_per_page' => 10,
	...
	public function get_smart_box_loop( $posts_query ) {
		$full_view_template = easyjet_get_locate_template( 'inc/widgets/news-smart-box/views/full-view.php' );
		$mini_view_template = easyjet_get_locate_template( 'inc/widgets/news-smart-box/views/mini-view.php' );
		...
		if ( ! $posts_query->have_posts() ) {
			return '<div class="container"><h4>' . esc_html__( 'Posts not found', 'easyjet' ) . '</h4></div>';
		}
		...
		while ( $posts_query->have_posts() ) : $posts_query->the_post();
			...
			$tags = $this->utility->meta_data->get_terms( array(
				...
				'type'      => 'post_tag',
				'delimiter' => ', ',
				'prefix'    => esc_html__( 'Tags: ', 'easyjet' ),
			...
			$image_size      = 'easyjet-thumb-560-315';
			$title_tag       = 'h4';
			...
			switch ( $this->instance['layout_type'] ) {
				case 'layout_type_1':
					if ( 1 <= $posts_query->current_post ) {
						...
						$image_size      = 'easyjet-thumb-260-147';
						$title_tag       = 'h5';
					} else {
	...
	public function new_smart_box_instance(){
		if ( ! isset( $_GET['nonce'] ) || ! wp_verify_nonce( $_GET['nonce'], 'easyjet-new-smart-box-nonce' ) ) {
			exit();
		}
...
add_action( 'widgets_init', 'easyjet_register_news_smart_box_widgets' );
/**
 * Register news-smart-box widget.
 */
function easyjet_register_news_smart_box_widgets() {
	register_widget( 'Easyjet_News_Smart_Box_Widget' );
}
